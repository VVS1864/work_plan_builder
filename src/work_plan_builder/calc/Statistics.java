...
public class Statistics {
	...
	public LinkedHashMap<LocalDateTime, List<Turn>> stillage_calendar = new LinkedHashMap<>();
	...
	public Statistics(Turns turns) {
		...
		for (LocalDateTime day = start; !(day.isAfter(end)); day = day.plusDays(1)) {
			...
			stillage_calendar.put(day, new ArrayList<Turn>());
				...
				for (Work_process proc : t.get_list_of_operation()) {
					...
					if ((day.isAfter(proc_start) || day.isEqual(proc_start))
						...
						if (proc.get_name().equals(Work_process.first_proc_name)) {
							...
							stillage_calendar.get(day).add(t);
						}
						// Temporal hardcode
					}
				}
	...
	public int get_stillage_units(LocalDateTime day) {
		return get_units(stillage_calendar, day);
	}
	...
	public String get_string() {
		...
		for (LocalDateTime day : chamber_calendar.keySet()) {
			...
			for (Turn t : stillage_calendar.get(day)) {
				ls_day_stillage.add(
